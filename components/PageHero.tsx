...
export function PageHero({ eyebrow, title, description, image, children }: PageHeroProps) {
  return (
    <section className="relative overflow-hidden bg-radiantInk py-24 text-white sm:py-28">
      <Image src={image} alt="" fill priority sizes="100vw" className="object-cover object-top opacity-80" />
      ...
      <div className="container-sr relative">
        ...
        {description ? <p className="mt-6 max-w-2xl text-lg leading-8 text-white/90">{description}</p> : null}
        {children}
      </div>
    </section>
  );
